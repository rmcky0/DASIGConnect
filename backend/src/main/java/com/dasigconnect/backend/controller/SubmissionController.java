...
public class SubmissionController {
     ...
     * Returns form reference data (allowed file types, size limits, guard rail thresholds).
     * Must be declared before /{id} mappings to avoid path ambiguity.
     */
    @GetMapping("/lookups")
     ...
     * Creates a new DRAFT submission. If scheduledAt is provided, reserves the slot.
     */
    @PostMapping
     ...
     * Returns full submission detail. Accessible by owner, institution validator, or admin.
     */
    @GetMapping("/{id}")
    @PreAuthorize("isAuthenticated()")
     ...
     * Partial update of a DRAFT or NEEDS_REVISION submission. Supports 60-second auto-save.
     */
    @PatchMapping("/{id}")
    @PreAuthorize("hasRole('CONTRIBUTOR')")
     ...
     * Transitions DRAFT → PENDING or NEEDS_REVISION → PENDING.
     ...
     * Called in real time by the SlotPicker component.
     */
     ...
     * the resulting storageUrl, fileName, fileType, and fileSizeBytes here.
     */
